      ...
      if (refreshToken) {
        ...
        if (refreshResponse.ok) {
          ...
          if (typeof access === "string" && typeof refreshToken === "string") {
            ...
            console.error("토큰 갱신 응답에서 유효하지 않은 토큰 타입:", {
              access,
              refreshToken,
            });
...
export const api = {
  ...
  lostItems: {
    // 분실물 신고
    create: (
      itemData: CreateLostItemRequest
    ): Promise<LostItemCreateResponse> =>
      apiRequest("/api/lost-items/", {
        // URL 수정: 끝에 슬래시 추가
        method: "POST",
    ...
    getAll: (params?: {
      ...
      return apiRequest(
        `/api/lost-items/list${queryString ? `?${queryString}` : ""}`
      );
    },
    ...
    update: (
      id: number,
      itemData: UpdateLostItemRequest
    ): Promise<LostItemUpdateResponse> =>
    ...
    ): Promise<LostItemStatusResponse> =>
  ...
  foundItems: {
    // 습득물 등록 (새로운 명세서에 맞춤)
    create: (
      itemData: CreateFoundItemRequest
    ): Promise<FoundItemCreateResponse> =>
    ...
    getAll: (params?: FoundItemListParams): Promise<FoundItemListResponse> => {
      ...
      const queryString = searchParams.toString();
      return apiRequest(
        `/api/found-items/list${queryString ? `?${queryString}` : ""}`
      );
    },
    ...
    update: (
      id: number,
      itemData: UpdateFoundItemRequest
    ): Promise<FoundItemCreateResponse> =>
      apiRequest(`/api/found-items/${id}/`, {
        method: "PUT",
        body: JSON.stringify(itemData),
    ...
    ): Promise<FoundItemStatusResponse> =>
    ...
    delete: (id: number): Promise<FoundItemDeleteResponse> =>
      ...
      // api.foundItems 아래에 추가
  getByCategories: (ids: number[]): Promise<FoundItemListResponse> => {
    const listA = `/api/found-items/list?category=${encodeURIComponent(
      ids.join(",")
    )}`;
    const listB = `/api/found-items/list?${ids
      .map((id) => `category=${id}`)
      .join("&")}`;

    // 백엔드가 어떤 걸 받는지 모르면 A 시도 후 실패 시 B 폴백
    return apiRequest(listA).catch(() => apiRequest(listB));
  },
  },
  ...
  police: {
    getFoundItems: (params?: { page?: number; limit?: number }) => {
      ...
      if (params?.limit)
        searchParams.append("numOfRows", params.limit.toString());
      const queryString = searchParams.toString();
      return apiRequest(`/api/police/found-items/?${queryString}`);
    },
    getLostItems: (params?: { page?: number; limit?: number }) => {
      const searchParams = new URLSearchParams();
      if (params?.page) searchParams.append("pageNo", params.page.toString());
      if (params?.limit)
        searchParams.append("numOfRows", params.limit.toString());
